...
Dashboard access tokens remain short-lived and are renewed from a stable session credential backed by a revocable database record. Protected API requests transparently recover an expired access cookie from that stable session, so page and server requests do not depend on a client-side refresh race. Renewal extends the session's rolling 30-day lifetime without rotating its credential, so simultaneous requests and browser tabs cannot invalidate the login session. A browser can restore a missing or stale CSRF cookie without discarding its valid HTTP-only session. Temporary database or service failures remain server errors and do not clear an otherwise valid session. Logout revokes the database session and clears every authentication cookie; invalid, expired, revoked, or disabled-account sessions are also cleared during renewal.
